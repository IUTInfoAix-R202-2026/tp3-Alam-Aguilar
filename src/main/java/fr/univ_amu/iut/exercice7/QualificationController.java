...
public class QualificationController {
  ...
  private void initialize() {
            ...
            (observable, oldValue, newValue) -> {
              if (newValue == null) {
                ...
                labelSelection.setText(
                    String.format(
                        "Séquence %s - %.1f kHz",
                        newValue.horodatageProperty().get().toString(),
                        newValue.frequenceDominanteKHzProperty().get()));
              }
            });

    // TODO exercice 7 (étape 3) : le bouton "Écouter" est désactivé tant qu'aucune
    // séquence n'est sélectionnée. Utiliser un binding entre
    // boutonEcouter.disableProperty()
    // et tableView.getSelectionModel().selectedItemProperty().isNull().
    boutonEcouter
        .disableProperty()
    ...
    // .otherwise(verdictGlobal.set("Verdict global
    // :"+verdictGlobalProperty.get())));
  ...
  private void ecouter() {
    ...
    if (selection == null) return; // si aucune ligne est selectionnee...
  ...
  private void enregistrerVerdict() {
    ...
    String verdictSelected = choiceBoxVerdict.getValue();
    if (verdictSelected == null || verdictSelected.isEmpty()) return;
    else nuit.setVerdictGlobal(verdictSelected);
  }
