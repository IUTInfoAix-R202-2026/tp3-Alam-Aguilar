...
public class ListeSitesController {
  ...
  public int ajouterSiteDemo() {
    ...
    int total = 0;
    compteurDemo++;
    SiteCarte newCarte = new SiteCarte();
    newCarte.setNumeroCarre("Carré " + (640000 + compteurDemo));
    newCarte.setNomConvivial("📍 Site de démonstration #" + compteurDemo);
    newCarte.setNombrePoints((compteurDemo % 3) + 1);
    newCarte.setNombrePassages(compteurDemo * 2);
    newCarte.setJoursDepuisDernierPassage(compteurDemo * 4);
    conteneurCartes.getChildren().add(0, newCarte);
